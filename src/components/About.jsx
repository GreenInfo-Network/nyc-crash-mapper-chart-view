...
const About = () => (
  <div className="About scroll">
    ...
    <p>
      <strong>ANALYZING: TREND</strong> allows you to compare two selected areas’ performance
      against each other and to a citywide or borough-wide reference, while filtering by date range
      or type of crash. COMPARE lets you compare two time periods for a given area. RANK shows where
      one area or one intersection ranks in terms of safety in the context of all similar areas over
      the most recent two-year period.
    </p>
    <p>
      <strong>ENHANCED DATA</strong>: Collision data is updated from the NYC Open Data Portal daily
      or as soon as new data becomes available with a one month back refresh. NYC Crash Mapper uses
      vehicle collision data for New York City from August 2011 to the present. The data was
      aggregated and normalized from the NYC Open Data Portal{' '}
    ...
    <p>
      ...
      fatalities, and injuries) displayed in the bottom of the map view may differ from what is
      shown on the map when Filter By Boundary is set to "Citywide." When a Filter By Boundary other
      than "Citywide" is used, such as a "Community Board", the stats will only display counts for
      crashes that have been geocoded and are located within a selected boundary.
    </p>
    <p>
      In the NYPD motor vehicles collisions dataset, some injuries and fatalities do not have a
      role/vehicle ascribed to them. This tool only reports injuries and fatalities which are
      attributed to a pedestrian, cyclist, or motorist. As a result the pedestrian/cyclist/motorist
      numbers displayed in this tool may not add up to the exact total in CrashMapper and OpenData.
    </p>
    <p>
      NYPD captures up to five (5) vehicles included in a crash. As long as any one of these vehicles
      matches the filter selected, the related injuries and fatalities will be counted – even if the vehicle
      is potentially not directly responsible for the fatalities/injuries.
    </p>
    <p>
